...
int main(int argc, char* argv[])
{
	...
	std::vector<unsigned long long> inspections(monkeys.size(),0);
	
	const auto fetch = [](auto type, auto old) -> unsigned long long
	{
		if(type=="old")
			return old;
	...
	const auto eval = [&](const auto& m, auto value)
	{
	...
	for(int i=0;i<10000;++i)
	{
		for(unsigned m_idx = 0;m_idx<monkeys.size();++m_idx)
		{
			...
			{
				++inspections[m_idx];
				item = eval(m,item)%modulus;
				monkeys[m.targets[item%m.divisor==0]].items.push_back(item);
			}
			m.items.clear();
		}
	}
	std::nth_element(inspections.begin(),inspections.begin()+2,inspections.end(),std::greater<>{});
