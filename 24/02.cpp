...
int main(int argc, char* argv[])
{
	...
	const auto min_time = [&](point2d from, point2d to, int start_step)
	{
		
		const auto compare = [&](const waypoint& lhs, const waypoint& rhs)
		{
			return rhs.steps+manhatten_dist(rhs.pos,to)<lhs.steps+manhatten_dist(lhs.pos,to) ||
			(rhs.steps+manhatten_dist(rhs.pos,to)==lhs.steps+manhatten_dist(lhs.pos,to) && manhatten_dist(rhs.pos,to)<manhatten_dist(lhs.pos,to));
		};
	...
	int first = min_time(start,end,0);
	int second = min_time(end,start,first);
	std::cout<<min_time(start,end,second);
	return 0;
}
